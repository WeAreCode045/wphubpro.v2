...
const LocalUploader: React.FC = () => {
    ...
    const isLimitReached = usage && subscription && usage.libraryUsed >= subscription.libraryLimit;

    if (isLimitReached) {
        return (
            <div className="text-center p-8 bg-card border border-border rounded-lg">
                <AlertTriangle className="mx-auto h-12 w-12 text-yellow-500" />
                <h3 className="mt-2 text-lg font-medium text-foreground">Library Limit Reached</h3>
                <p className="mt-1 text-sm text-muted-foreground">
                    You have reached your limit of {subscription?.libraryLimit} items. Please upgrade your plan to add more.
                </p>
    ...
    return (
        <Card className="max-w-2xl mx-auto">
            <CardHeader>
                ...
                <CardDescription>Upload a .zip file to add it to your local library.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <label className="text-sm font-medium">Item Type</label>
                    <div className="flex gap-4 mt-2">
                        <Button onClick={() => setType(LibraryItemType.Plugin)} variant={type === 'plugin' ? 'default' : 'outline'}>Plugin</Button>
                        <Button onClick={() => setType(LibraryItemType.Theme)} variant={type === 'theme' ? 'default' : 'outline'}>Theme</Button>
                    </div>
                </div>

                {!file ? (
                     <div {...getRootProps()} className={`p-10 border-2 border-dashed rounded-lg cursor-pointer text-center ${isDragActive ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'}`}>
                        <input {...getInputProps()} />
                        <UploadCloud className="mx-auto h-12 w-12 text-muted-foreground" />
                        <p className="mt-2 text-sm text-muted-foreground">
                            {isDragActive ? 'Drop the file here...' : 'Drag & drop a .zip file here, or click to select'}
                        </p>
                    </div>
                ) : (
                    <div className="p-4 border rounded-lg flex items-center justify-between bg-secondary">
                       ...
                       <Button variant="ghost" size="icon" onClick={() => { setFile(null); uploadMutation.reset(); }}><X className="w-4 h-4" /></Button>
                    </div>
                )}
