    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2">
                ...
                <Button variant={filter === 'plugin' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('plugin')}>Plugins</Button>
                <Button variant={filter === 'theme' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('theme')}>Themes</Button>
            </div>
                        ...
                        <div key={item.$id} className="bg-card border border-border rounded-lg p-4 flex flex-col justify-between">
                            <div>
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${item.source === 'local' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                                        {item.source}
                                    </span>
                                </div>
                            ...
                            <div className="flex justify-between items-center mt-4 pt-4 border-t border-border">
                                <span className="text-xs font-mono bg-secondary px-2 py-1 rounded">v{item.version}</span>
