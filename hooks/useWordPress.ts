...
const executeWpProxy = async <T>(payload: { siteId: string; method?: string; endpoint: string; body?: any; }): Promise<T> => {
  try {
    ...
    if (payload.method) qs.set('method', payload.method);
    ...
    if ((payload as any).userId) qs.set('userId', (payload as any).userId);

    const path = `/?${qs.toString()}`;

    const result = await functions.createExecution(FUNCTION_ID, undefined, false, path, payload.method || 'GET');
